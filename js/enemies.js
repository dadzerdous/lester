...
function spawnEnemy() {
  ...
  // Swooper spawns high — at ~2 head-heights above player, wherever player currently is.
  // It oscillates down to player head level periodically (the dip to dodge).
  const spawnY = isSwooper ? player.y - PH*2 - rnd(10,30) : GY() - t.h/2;
  enemies.push({
    x: spawnX,
    y: spawnY,
    ...
    swoopTimer: rnd(800,2000), // ms before first dive
    swooping: false,           // true while actively diving
    locked:   false,           // true once swooper has reached its target height and flies flat
    baseY: spawnY,             // hover altitude before locking
    nameTimer: 1800,
  });
}
...
function updateEnemies(dt) {
  ...
  for (let i = enemies.length-1; i >= 0; i--) {
    ...
    if(e.label === 'swooper'){
      // Swooper hovers at ~2 head-heights above player, then dips down to
      // 1 head-height (player head level) every swoopTimer ms, then rises back.
      // Design: player can jump OVER the dip, or SLIDE under it.
      // "Head height" = PH/2 (distance from player centre to top of head).
      const headH = PH / 2;
      const highY = player.y - headH * 3.5;  // hover altitude: ~2 heads above player head
      const lowY  = player.y - headH * 1.0;  // dip altitude: at player head level

      e.swoopTimer -= dt;
      if(!e.swooping && e.swoopTimer <= 0){
        e.swooping = true;  // begin dip downward
      }

      if(e.swooping){
        // Dipping toward player head level
        e.y += 3.5;
        if(e.y >= lowY){
          e.y = lowY;
          e.swooping = false;
          e.swoopTimer = rnd(1800, 3200); // wait before next dip
        }
      } else {
        // Rising back to hover altitude
        if(e.y > highY + 4) e.y -= 2.5;
        else e.y = highY + Math.sin(e.anim * 1.5) * 6; // gentle hover bob
      }
      // Clamp so it never goes above canvas top or below ground
      e.y = Math.max(20, Math.min(GY() - e.h - 10, e.y));
    }
    ...
    if (gameState === 'playing') {
      const m=6;
      const ph = player.crouching ? PH*0.6 : PH;
